...
def testSift(frame,lframe):
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    lgray = cv2.cvtColor(lframe, cv2.COLOR_BGR2GRAY)

    sift = cv2.xfeatures2d.SIFT_create()
    lsift = cv2.xfeatures2d.SIFT_create()

    (kps, descs) = sift.detectAndCompute(gray, None)

    (lkps, ldescs) = lsift.detectAndCompute(lgray, None)

    bf = cv2.BFMatcher()
    matches = bf.match(descs, ldescs)
    ...
    print("# lkps: {}, ;descriptors: {}".format(len(lkps), ldescs.shape))

    surf = cv2.xfeatures2d.SURF_create()
    lsurf = cv2.xfeatures2d.SURF_create()

    (kps, descs) = surf.detectAndCompute(gray, None)
    (lkps, ldescs) = lsurf.detectAndCompute(lgray, None)

    print("# kps: {}, descriptors: {}".format(len(kps), descs.shape))
    print("# lkps: {}, ldescriptors: {}".format(len(lkps), ldescs.shape))
